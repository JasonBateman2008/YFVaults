...
const images = {
  ...
  xtz: {
    url: 'https://bscscan.com/token/images/tezos_32.png',
    rounded: true,
    bg: 'white'
  },
  ten: {
    url: 'https://bscscan.com/token/images/tenet_32.png',
    rounded: true,
  },
  balbt: {
    url: 'https://bscscan.com/token/images/allianceblock_32.png',
  },
  swgb: {
    url: 'https://bscscan.com/token/images/swirge_32.png',
    rounded: true,
  },
  zee: {
  ...
  bry: {
    url: 'https://bscscan.com/token/images/berrydata_32.png',
  },
  swingby: {
...
const TokenIcon = ({ token, ...rest}) => {
  if (!token) {
    return null
  }
  const { url, rounded, bg, darkBg, margin } = token
  return (
    <img
      src={url}
      alt=""
...
const Pool = ({
  ...
  const [ pid, /*multiplier*/, asset, , tvl, apy] = summary

  const [token1, token2] = useMemo(() => {
    return asset.toLowerCase().replace(' lp', '').split('-')
  }, [asset])
        ...
        <Card className={classes.root}>
          <CardContent className={classes.cover}>
            <div className={classes.tokens}>
              <div className="flex">
                <TokenIcon token={images[token1]} />
                { token2
                  ? <TokenIcon style={{ marginLeft: '-0.75rem', maxHeight: 44 }} token={images[token2]} />
                  : <div style={{ marginLeft: '-0.75rem'}} className="w-6 sm:w-8" />
                }
              </div>
            </div>
          </CardContent>
          <CardContent className={classes.content}>
            <Grid container direction="row" justify="space-between">
              <Grid item className="w-12 text-gray-500">
                <label>ASSET</label>
              </Grid>
              <Grid item>
                <span>{ asset }</span>
