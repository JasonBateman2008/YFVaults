...
const options = {
  ...
  headers: {
    authorization: 'Basic ' + Buffer.from(':1e3da0e58fa547f896882fc194065656','utf8').toString('base64')
  },
...
// }).then(result => {
//   for (const { raw, blockNumber } of result) {
//     const { interestAccumulated, borrowIndex, totalBorrows } = decodeLog(raw);

//     console.log('interestAccumulated = %s, borrowIndex = %s, totalBorrows = %s', interestAccumulated, borrowIndex, totalBorrows);
...
//     // ])
//     // .then(([ borrowRate, supplyRate ]) => {
//     //   console.log(borrowRate, utils.fromWei(supplyRate))
//     // })
//   }
// })

web3Eth.subscribe('newBlockHeaders').on('data', ({ number }) => {
  toBlock = number;

  contract
  .methods
  .balanceOf('0x299d3a20ec7785f1b6617fa9c916898b7a6f2c7d')
  .call({}, number)
  .then(token => {
    console.log('cETH = %s', token)
  })

  Promise.all([
    contract.methods.borrowRatePerBlock().call({ from: '0x299d3a20ec7785f1b6617fa9c916898b7a6f2c7d' }, number),
    contract.methods.supplyRatePerBlock().call({ from: '0x299d3a20ec7785f1b6617fa9c916898b7a6f2c7d' }, number),
    contract.methods.exchangeRateCurrent().call({}, number),
  ])
  .then(([ borrowRate, supplyRate, exchangeRate ]) => {
    console.log('supplyRate = %s, exchangeRate = %s, at %s block', utils.fromWei(supplyRate), exchangeRate, number)
  })

  contract
    .methods
    .balanceOfUnderlying('0x299d3a20ec7785f1b6617fa9c916898b7a6f2c7d')
    .call({}, number)
    .then(cash => {
      if (cashPrior) {
        console.log('got supplyRate: %s', Decimal(cash).minus(cashPrior).div(cashPrior).toFixed(18))
        console.log('supply_balance_underlying = %s\n\n', cash)
      }
      cashPrior = cash;
    })
});
